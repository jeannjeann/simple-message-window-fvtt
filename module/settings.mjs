...
class LayoutSettingsDialog extends FormApplication {
  ...
  _animateText(element, text, onComplete) {
    const speedValue =
      this.form.querySelector('[name="textSpeed"]')?.value ?? 100;
      ...
      initial: {
        ...
        windowHPos: parseFloat(
          this.form.querySelector(
            'input[type="number"][name="windowHPosition"]'
          ).value
        ),
        windowVPos: parseFloat(
          this.form.querySelector(
            'input[type="number"][name="windowVPosition"]'
          ).value
        ),
        windowWidth: parseFloat(
          this.form.querySelector('input[type="number"][name="windowWidth"]')
            .value
        ),
        windowHeight: parseFloat(
          this.form.querySelector('input[type="number"][name="windowHeight"]')
            .value
        ),
        imgSize: parseFloat(
          this.form.querySelector('input[type="number"][name="imgSize"]').value
        ),
      },
    };
  ...
  _onDragMove(event) {
    ...
    if (action === "move-window") {
      ...
      Array.from(this.form.elements.windowHPosition).forEach(
        (el) => (el.value = newHPos)
      );
      Array.from(this.form.elements.windowVPosition).forEach(
        (el) => (el.value = newVPos)
      );
      ...
      Array.from(this.form.elements.windowWidth).forEach(
        (el) => (el.value = newWidth)
      );
      Array.from(this.form.elements.windowHeight).forEach(
      ...
      Array.from(this.form.elements.imgSize).forEach(
  ...
  async _onResetSettings(event) {
    ...
    for (const key of settingsToReset) {
      ...
      const inputs = this.form.elements[key];
      if (inputs)
